...
def predict_crime(emb_all):
    crime = np.load("./data/crime_counts_label.npy")
    y_pred, y_test = kf_predict(emb_all, crime)
...
def predict_checkin(emb_all):
    check_in = np.load("./data/check_in_label.npy")
    y_pred, y_test = kf_predict(emb_all, check_in)
    mae, rmse, r2 = compute_metrics(y_pred, y_test)
...
def lu_classify(emb):
    ...
    ari = adjusted_rand_score(cd_labels, emb_labels)
    f_score = F_meansure(cd_labels, emb_labels)
    print("nmi: {:.3f}".format(nmi), " | ars: {:.3f}".format(ari), " | f_score: {:.3f}".format(f_score))
